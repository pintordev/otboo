...
public interface LocationRepository extends JpaRepository<Location, UUID> {
  ...
  @Modifying
  @Query(value = """
      INSERT INTO locations (id, x, y, latitude, longitude, location_names, created_at, updated_at)
      VALUES (:id, :x, :y, :latitude, :longitude, CAST(:locationNamesJson AS jsonb), now(), now())
      ON CONFLICT (x, y) DO NOTHING
      """, nativeQuery = true)
  void insertIfAbsent(@Param("id") UUID id, @Param("x") int x, @Param("y") int y,
      @Param("latitude") double latitude, @Param("longitude") double longitude,
      @Param("locationNamesJson") String locationNamesJson);
}