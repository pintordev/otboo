...
class LogBackupStepListenerTest {

  private LogBackupStepListener listener;
  ...
  @BeforeEach
  void setUp() {
    listener = new LogBackupStepListener(logBackupMetrics);
    logger = (Logger) LoggerFactory.getLogger(LogBackupStepListener.class);
  ...
  class AfterStep {
    ...
    void 시작_종료_시각이_있으면_처리건수와_소요시간을_로그로_남기고_ExitStatus를_그대로_반환한다() {
      // given
      given(stepExecution.getStartTime()).willReturn(LocalDateTime.of(2026, 8, 28, 1, 0, 0));
      given(stepExecution.getEndTime()).willReturn(LocalDateTime.of(2026, 8, 28, 1, 0, 5));
    ...
    void 시작_종료_시각이_있으면_skipCount를_카운터에_기록한다() {
      ...
      // then
      verify(logBackupMetrics).countStepSkipped(1L);
    }
