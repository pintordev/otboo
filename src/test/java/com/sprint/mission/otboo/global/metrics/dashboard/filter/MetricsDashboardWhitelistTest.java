...
class MetricsDashboardWhitelistTest {
  ...
  MetricsDashboardWhitelist whitelist =
      new MetricsDashboardWhitelist(List.of("batch.", "recommendation.cache."));
  ...
  class Matches {
    ...
    void 설정된_프리픽스로_시작하면_허용한다() {
      assertThat(whitelist.matches("batch.weather-fetch.job.completed")).isTrue();
      assertThat(whitelist.matches("recommendation.cache.hit")).isTrue();
    }

    @Test
    @DisplayName("설정된 프리픽스가 아니면 거부한다")
    void 설정된_프리픽스가_아니면_거부한다() {
      assertThat(whitelist.matches("jvm.memory.used")).isFalse();
    }
  }
}